"""
Tests para finalize_inbound_pstn_end: cierre unificado PSTN inbound (ChannelDestroyed vs StasisEnd).
"""
import os
import sys
import threading
...
def _queue_context(call_id: str = "inbound-1", pstn_channel: str = "pstn-ch-1") -> CallContext:
    return CallContext(
        call_id=call_id,
        type=StateCallType.INBOUND,
...
class TestFinalizeInboundPstnEnd(unittest.TestCase):
    def test_channel_destroyed_reports_exit_abandon_when_wins_mark(self):
        ...
        state_store.mark_call_ended_atomic.assert_called_once_with("inbound-1")
        reporter.log_segment_end.assert_called_once()
        self.assertEqual(
            reporter.log_segment_end.call_args.kwargs.get("event_final"),
            "EXIT_ABANDON",
        )
        state_store.unregister.assert_called_once_with("inbound-1")
        handler.queue_event_manager.on_abandon.assert_called_once()

    def test_stasis_end_reports_exit_abandon_when_wins_mark(self):
        state_store = MagicMock()
        reporter = MagicMock()
        context = _queue_context()
        state_store.get_by_channel.return_value = context
    ...
    def test_race_only_one_report(self):
        """Simula carrera: solo el thread que gana mark debe reportar."""
        state_store = MagicMock()
        reporter = MagicMock()
        context = _queue_context()
    ...
    def test_safety_net_runs_even_when_loses_mark(self):
        ...
        dist.stop_distribution.assert_called_once_with("inbound-1")
